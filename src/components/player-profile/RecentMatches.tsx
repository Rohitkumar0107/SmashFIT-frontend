          ...
          {matches.map((match: any) => (
            ...
            >
              <div className="flex items-center gap-4">
                {/* W/L Badge */}
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-lg shadow-sm ${match.result === 'W' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200'
                  }`}>
                  {match.result}
                </div>
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">
                    {match.category?.replace('_', ' ') || ''} {match.category ? '•' : ''} {match.match_date ? new Date(match.match_date).toLocaleDateString([], { day: 'numeric', month: 'short' }) : ''}
                  </p>
                </div>
              </div>
