    ...
    <div className="max-w-5xl mx-auto animate-in fade-in duration-500 pb-12 px-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <button 
          ...
          className="flex items-center gap-2 text-slate-500 hover:text-slate-900 font-bold bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm transition-all hover:shadow-md w-fit"
        >
          <ArrowLeft size={18} />
          Back to List
        </button>
        ...
        {isOwner && (
          <div className="flex items-center gap-3 animate-in slide-in-from-right-5 duration-300">
            <Link 
              ...
              className="flex items-center gap-2 text-slate-700 hover:text-slate-900 font-bold bg-slate-100 hover:bg-slate-200 px-4 py-2.5 rounded-xl transition-colors border border-slate-200"
            >
              <Edit3 size={18} />
              Edit Profile
            </Link>
            ...
            <button 
              ...
              className="flex items-center gap-2 text-white font-bold bg-blue-600 hover:bg-blue-700 px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-blue-600/20 active:scale-95"
            >
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden relative">
        ...
        <div className="px-6 md:px-10 pb-10">
          <div className="relative flex justify-between items-end -mt-16 mb-6">
            <div className="w-32 h-32 rounded-2xl bg-white p-2 shadow-lg border border-slate-100">
              <div className="w-full h-full rounded-xl bg-slate-100 flex items-center justify-center overflow-hidden">
                {!imgError && org.logo_url ? (
                  ...
                  <span className="text-5xl font-black text-slate-300">{org.name.charAt(0).toUpperCase()}</span>
                )}
              </div>
            </div>
            <div className="mb-4">
              <span className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-black tracking-wide ${org.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                <CheckCircle2 size={16} /> {org.status}
              </span>
            </div>
          </div>

          <h1 className="text-3xl md:text-4xl font-black text-slate-900 mb-2">{org.name}</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            <DetailCard icon={<Layers className="text-indigo-500"/>} label="Total Courts" value={`${org.court_count || 0} Courts`} />
            <DetailCard icon={<Building className="text-emerald-500"/>} label="Flooring Type" value={org.flooring_type || 'Standard'} />
            <DetailCard icon={<Mail className="text-sky-500"/>} label="Business Email" value={org.business_email || 'Not provided'} />
            <DetailCard icon={<FileText className="text-amber-500"/>} label="GST Number" value={org.gst_number || 'N/A'} />
          </div>

          {/* 🛁 Amenities Section */}
          {org.amenities && Object.keys(org.amenities).length > 0 && (
            <div>
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-4">Facilities Available</h3>
              <div className="flex flex-wrap gap-3">
                {org.amenities.ac && <AmenityBadge icon={<Wind size={18}/>} label="Air Conditioning" color="sky" />}
                {org.amenities.parking && <AmenityBadge icon={<CarFront size={18}/>} label="Parking Available" color="slate" />}
                {org.amenities.shower && <AmenityBadge icon={<Bath size={18}/>} label="Shower Rooms" color="indigo" />}
                {org.amenities.water && <AmenityBadge icon={<Droplets size={18}/>} label="Drinking Water" color="blue" />}
              </div>
            </div>
          )}
        </div>
...
const DetailCard = ({ icon, label, value }: any) => (
  <div className="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
    ...
    <p className="text-slate-800 font-bold truncate pl-1">{value}</p>
  </div>
);
...
const AmenityBadge = ({ icon, label, color }: any) => {
  ...
  return (
    <div className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border font-bold text-sm ${colorClasses[color]}`}>
      {icon} {label}
    </div>
  );
};
